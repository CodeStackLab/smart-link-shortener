...
console.log('🧪 Starting Super Admin Domains Only Verification (Problem 1)...\n');
...
test('2. admin.html has #custom-domains-card hidden by default and hides domains permission checkboxes', () => {
...
test('3. dashboard.js adds is-superadmin class and strictly restricts customDomainsCard and loadDomains to isSuperAdminUser', () => {
  const js = fs.readFileSync('./public/js/dashboard.js', 'utf8');
  assert(js.includes("document.body.classList.add('is-superadmin')"), 'Must add is-superadmin class to body for superadmin');
  assert(js.includes("document.body.classList.remove('is-superadmin')"), 'Must remove is-superadmin class for non-superadmin');
...
test('4. server.js protects /api/admin/domains with requireSuperAdmin and filters domains from getUserPermissions', () => {
  ...
  assert(serverJs.includes("p !== 'domains'"), 'getUserPermissions must strip domains from non-superadmin users');
});
